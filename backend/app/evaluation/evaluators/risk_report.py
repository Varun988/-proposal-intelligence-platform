...
class RiskReportEvaluator(
    ...
    def evaluate(
        ...
        if critical_count or high_count:
            ...
            summary = "The Risk and Report Agent output completed with non-blocking warnings."
        else:
            status = EvaluationStatus.PASSED
            summary = (
                "The Risk and Report Agent output passed all deterministic validation controls."
            )
        ...
        return EvaluationResult(
            ...
            metrics={
                ...
                "source_proposal_finding_count": len(target.source_proposal_finding_ids),
                "source_vendor_finding_count": len(target.source_vendor_finding_ids),
                "referenced_proposal_finding_count": len(referenced_proposal_findings),
                "referenced_vendor_finding_count": len(referenced_vendor_findings),
                "human_review_required": (target.result.human_review_required),
                "official_decision_provided": (target.result.official_decision_provided),
                "instruction_version_matches": (
                    target.instruction_version == RISK_REPORT_INSTRUCTION_VERSION
                ),
            },
        )
    ...
    def _evaluate_instruction_version(
        ...
        if target.instruction_version != RISK_REPORT_INSTRUCTION_VERSION:
            findings.append(
                EvaluationFinding(
                    finding_id=("risk-report-instruction-version-mismatch"),
                    description=(
                        "The Risk and Report Agent used an unexpected instruction version."
                    ),
                    severity=EvaluationSeverity.HIGH,
                    location="instruction_version",
                    metadata={
                        "actual_version": (target.instruction_version),
                        "expected_version": (RISK_REPORT_INSTRUCTION_VERSION),
                    },
                )
            )
    ...
    def _evaluate_governance(
        ...
        if not target.result.human_review_required:
            findings.append(
                EvaluationFinding(
                    ...
                    description=("The Risk and Report output does not require human review."),
        ...
        if target.result.official_decision_provided:
            findings.append(
                EvaluationFinding(
                    finding_id=("risk-report-official-decision-provided"),
                    description=("The Risk and Report Agent produced an official vendor decision."),
                    severity=EvaluationSeverity.CRITICAL,
                    location=("result.official_decision_provided"),
                )
            )

        if target.result.executive_report.decision_disclaimer != RISK_REPORT_DECISION_DISCLAIMER:
            findings.append(
                EvaluationFinding(
                    finding_id=("risk-report-disclaimer-mismatch"),
                    description=(
                        "The executive report does not contain "
                        "the required decision-support disclaimer."
                    ),
                    severity=EvaluationSeverity.CRITICAL,
                    location=("result.executive_report.decision_disclaimer"),
                )
            )
    ...
    def _evaluate_risks(
        ...
        allowed_source_agents = set(RISK_REPORT_ALLOWED_SOURCE_AGENTS)
        proposal_finding_ids = set(target.source_proposal_finding_ids)
        vendor_finding_ids = set(target.source_vendor_finding_ids)

        seen_risk_ids: set[str] = set()

        for risk_index, risk in enumerate(target.result.risks):
            location = f"result.risks[{risk_index}]"

            if risk.risk_id in seen_risk_ids:
                findings.append(
                    EvaluationFinding(
                        finding_id=(f"duplicate-risk-id-{risk.risk_id}"),
                        description=("The Risk and Report output contains a duplicate risk ID."),
                        severity=EvaluationSeverity.HIGH,
                        location=location,
            ...
            if not risk.human_review_required:
                findings.append(
                    EvaluationFinding(
                        finding_id=(f"risk-human-review-disabled-{risk.risk_id}"),
                        description=(
                            "An individual synthesized risk does not require human review."
                        ),
                        severity=EvaluationSeverity.HIGH,
                        location=(f"{location}.human_review_required"),
                        related_ids=[risk.risk_id],
                    )
                )
            ...
            for evidence_index, evidence in enumerate(risk.evidence):
                ...
                if evidence.source_agent not in allowed_source_agents:
                    findings.append(
                        EvaluationFinding(
                            finding_id=(
                                f"unauthorized-risk-source-agent-{risk.risk_id}-{evidence_index}"
                            ),
                            description=("Risk evidence references an unauthorized source agent."),
                            severity=EvaluationSeverity.CRITICAL,
                    ...
                    if evidence.source_finding_id not in proposal_finding_ids:
                        findings.append(
                            EvaluationFinding(
                                ...
                                description=(
                                    "Risk evidence references an unknown Proposal Analysis finding."
                                ),
                                severity=(EvaluationSeverity.CRITICAL),
                                location=evidence_location,
                                related_ids=[
                    ...
                    if evidence.proposal_evidence is None:
                        findings.append(
                            EvaluationFinding(
                                ...
                                description=(
                                    "Proposal Analysis source does not contain proposal evidence."
                                ),
                                severity=(EvaluationSeverity.CRITICAL),
                                location=evidence_location,
                            )
                ...
                if evidence.source_agent == "vendor-research":
                    if evidence.source_finding_id not in vendor_finding_ids:
                        findings.append(
                            EvaluationFinding(
                                ...
                                description=(
                                    "Risk evidence references an unknown Vendor Research finding."
                                ),
                                severity=(EvaluationSeverity.CRITICAL),
                                location=evidence_location,
                                related_ids=[
                    ...
                    if evidence.vendor_evidence is None:
                        findings.append(
                            EvaluationFinding(
                                finding_id=(
                                    f"missing-vendor-risk-evidence-{risk.risk_id}-{evidence_index}"
                                ),
                                description=(
                                    "Vendor Research source does not contain vendor evidence."
                                ),
                                severity=(EvaluationSeverity.CRITICAL),
                                location=evidence_location,
                            )
            ...
            if risk.deterministic_score is not None:
                if not risk.deterministic_score.contributing_rule_ids:
                    findings.append(
                        EvaluationFinding(
                            finding_id=(f"risk-score-without-rules-{risk.risk_id}"),
                            description=(
                                "A deterministic risk score does "
                                "not identify contributing rule IDs."
                            ),
                            severity=EvaluationSeverity.HIGH,
                            location=(f"{location}.deterministic_score"),
                            related_ids=[risk.risk_id],
                        )
                    )
    ...
    def _evaluate_report_references(
        ...
        risk_ids = {risk.risk_id for risk in target.result.risks}
        ...
        for section_index, section in enumerate(target.result.reviewer_report.sections):
            location = f"result.reviewer_report.sections[{section_index}]"

            if section.section_id in seen_section_ids:
                findings.append(
                    EvaluationFinding(
                        finding_id=(f"duplicate-report-section-{section.section_id}"),
                        description=("The reviewer report contains a duplicate section ID."),
            ...
            unknown_risk_ids = set(section.related_risk_ids) - risk_ids

            if unknown_risk_ids:
                findings.append(
                    EvaluationFinding(
                        finding_id=(f"unknown-report-risk-reference-{section.section_id}"),
                        description=(
                        ...
                        severity=EvaluationSeverity.HIGH,
                        location=location,
                        related_ids=sorted(unknown_risk_ids),
                    )
                )

            unknown_finding_ids = set(section.related_finding_ids) - source_finding_ids

            if unknown_finding_ids:
                findings.append(
                    EvaluationFinding(
                        finding_id=(f"unknown-report-finding-reference-{section.section_id}"),
                        description=(
                        ...
                        severity=EvaluationSeverity.HIGH,
                        location=location,
                        related_ids=sorted(unknown_finding_ids),
                    )
                )
